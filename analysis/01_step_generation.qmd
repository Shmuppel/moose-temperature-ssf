...
library(tidyverse)
packages <- c("amt", "lubridate", "tictoc", "sf", "ggspatial")
...
moose <- moose %>%
  ...
  mutate(date = force_tz(ymd_hms(date), tzone="Europe/Stockholm")) %>%
  ungroup()
```
...
```{r}
ggplot(moose_strata_distribution, aes(x=strata_percentage)) +
  geom_histogram(aes(fill = ..count..)) +
  scale_fill_viridis_c(option = "plasma") +
