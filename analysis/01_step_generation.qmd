...
library(tidyverse)
packages <- c("amt", "lubridate", "sf", "ggspatial", "glue")
...
ggplot(seasonal_coverage, aes(x=coverage)) +
  ...
  scale_fill_gradientn(colors = greens_to_browns(100)) +
  labs(
...
```{r}
# First, keep only the rows in seasonal_coverage that meet the threshold
seasonal_coverage_filtered <- seasonal_coverage %>% 
  filter(coverage >= threshold) %>% 
  rename(year = season_year)

# Then, filter moose to only include rows where animal_id and year are in the threshold
moose <- moose %>% semi_join(seasonal_coverage_filtered, by = c("animal_id", "year"))
```
...
plot1 <- moose %>% ggplot(aes(x = date, y = factor(strata), colour = factor(strata))) +
  ...
  scale_color_manual(
    ...
    "boreal inland" = green,
    "boreal coast" = mossgreen,
    "boreal alpine" = darkgreen
  )) +
  theme_minimal() +
  theme(
...
ggsave('figures/01_localisations_per_year.jpg', height=3, width=7)
plot1
```
...
n_localisations = nrow(moose_tracks)
n_animals = length(unique(moose_tracks$animal_id))
n_years <- length(unique(moose_tracks$year))

custom_green_brown <- greens_to_browns(n_years)

# plot the data, spatially colored by year
moose_sf <- st_as_sf(moose_tracks, coords=c("x_", "y_"), crs=st_crs(3006))
plot2 <- ggplot() +
  layer_spatial(strata, fill="transparent", color=grey, size=4, alpha=1) +
  theme_minimal()
ggsave('figures/01_strata_plain.jpg')
plot2
```
...
moose_sf <- st_as_sf(moose_tracks, coords=c("x_", "y_"), crs=st_crs(3006))
plot2 <- ggplot() +
  layer_spatial(strata, fill="transparent", color=grey, size=0.1, alpha=0.6) +
  layer_spatial(moose_sf, size=0.6, aes(color=as.factor(year))) +
  scale_color_manual(name="Year", values = custom_green_brown) + 
  labs(
    title="Localisations per Strata", 
    subtitle=glue("n localisations = {n_localisations}, n animals = {n_animals}")
  ) +
  theme_minimal()
