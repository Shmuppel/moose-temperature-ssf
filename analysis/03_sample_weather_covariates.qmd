...
readRenviron("../.env")
source('./functions/03_functions.r')
```
...
```{r}
start_date <- as.POSIXct("2017-06-01 00:00:00", tz="Europe/Stockholm")
end_date <- as.POSIXct("2017-06-08 00:00:00", tz="Europe/Stockholm")
...
```{r}
weather_data$year <- factor(year, levels=regression_model$xlevels$year)
weather_data$gtt <- temp_geom(weather_data$day, weather_data$lat, variable="mean")
...
sum_metric_model <- vgmST("sumMetric",
                          ...
                          joint= vgm(psill=15, "Sph",  range=500000, nugget=1),
...
-   moose_data: subset of moose_random_steps that have either t1\_ or t2\_ falling in the main window

-   weather_data: weather data for the extended window

```{r}
# We use all times from both start and end steps.
all_times <- c(moose_random_steps$t1_)
...
process_week_batch <- function(batch) {
  ...
  logger(glue("Processing {week_id}"))
  ...
  moose_data <- left_join(
    moose_data, unique_start_steps,
    by = c("x1_", "y1_", "t1_", "animal_id")
  )
  moose_data <- left_join(
    moose_data, end_steps,
  ...
  logger(paste0(
    as.character(moose_data[1, "t1_"]), "\n",
    moose_data[1, "temperature_start"], "\n",
    moose_data[1, "temperature_end"], "\n",
    moose_data[1, "collar_temperature"]))
  
  logger(glue("Finished processing {week_id}"))
}
```

The below code chunk runs the palatalization magic, warning it will take a day to run!

```{r}
backend <- start_backend(cores = 3, cluster_type = "fork")
...
# Function to create a result writer (CSV appender)
...
# Function to create a logger (writes text logs)
...
par_lapply(backend, week_batches, process_week_batch)
