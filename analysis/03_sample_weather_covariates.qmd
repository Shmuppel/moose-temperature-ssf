...
```{r}
start_date <- as.POSIXct(glue("2020-06-01 00:00:00"), tz="Europe/Stockholm")
end_date <- as.POSIXct(glue("2020-06-07 00:00:00"), tz="Europe/Stockholm")
...
process_week_batch <- function(batch) {
  ...
  start_processed <- prepare_start_steps(moose_data)
  end_processed   <- prepare_end_steps(moose_data)
  
  # Combine extended localisations from both steps.
  localisations <- bind_rows(start_processed, end_processed)
  
  print(head(localisations, 1))
}
```

```{r}

  # --- Prepare Main Window Data for Prediction ---
  # For start steps: select rows where t1_ falls in the main window.
  main_start_df <- moose_data %>% filter(t1_ >= main_start, t1_ <= main_end)
  main_start_processed <- prepare_start_steps(main_start_df)
  
  # For end steps: select rows where t2_ falls in the main window.
  main_end_df <- moose_data %>% filter(t2_ >= main_start, t2_ <= main_end)
  main_end_processed <- prepare_end_steps(main_end_df)
  
  # Optimize start-step predictions by deduplicating repeated locations.
  unique_start <- main_start_processed %>% distinct(x, y, time, elevation, day, lat)
  
  start_predictions_unique <- predict_weather_at_localisations(unique_start, kriging_model,
                                                               maxdist = 200000, nmax = 5)
  
  start_pred_df <- left_join(main_start_processed,
                             unique_start %>% mutate(prediction = start_predictions_unique),
                             by = c("x", "y", "time", "elevation", "day", "lat"))
  
  # Predict for end steps (each row is unique)
  end_predictions <- predict_weather_at_localisations(main_end_processed, kriging_model,
                                                      maxdist = 200000, nmax = 5)
  
  main_end_processed <- main_end_processed %>% mutate(prediction = end_predictions)
  
  # Tag the predictions with step type and week_id.
  start_pred_df <- start_pred_df %>% mutate(step = "start", week = batch$week_id)
  main_end_processed <- main_end_processed %>% mutate(step = "end", week = batch$week_id)
  
  # Print finishing message.
  cat("Finished week:", batch$week_id, "\n")
  flush.console()
  
  # Combine start and end predictions and return.
  result <- bind_rows(start_pred_df, main_end_processed)
  return(result)
```

```{r}
# ---- STEP 3: Process each week batch in parallel using parLapply ----
cl <- makeCluster(1, outfile='03_parallel_log.txt') 
